...
void CheckResidual(const GaussMatrix& A, const GaussVector& x, const GaussVector& b, double eps = 1e-6)
{
    ...
    EXPECT_LT(residual.norm(), eps) << "Residual norm is too large: " << residual.norm();
}
...
TEST(GaussSolve, SmallSystem_2x2_Handcrafted)
{
    ...
    CheckResidual(A, sol, b, 1e-8);
}

TEST(GaussSolve, IdentityMatrix_NxN)
{
    const int n = 10;
    GaussMatrix A = GaussMatrix::Identity(n, n);
    GaussVector b = GaussVector::LinSpaced(n, 1.0, 10.0); // b = [1, 2, ..., 10]
